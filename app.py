...
DEFAULT_XLSX = "wasteinfo.xlsx"   # 같은 폴더에 배치
...
def canon(s: str) -> str:
    """정확 비교용 정규화: 공백/하이픈 제거, 괄호 통일, 소문자"""
...
# 법령(정확 일치만)
# =========================
def get_ref_exact(df_ref: pd.DataFrame, name_or_alias: str) -> Optional[Dict[str, str]]:
    """별표5 '폐기물 종류'와 정확 일치(정규화 기준)하는 첫 행 반환"""
    key = canon(name_or_alias)
...
def propose_with_openai(client, waste_name: str, phase: Optional[str], material: str) -> str:
    ...
    user = {
        ...
        "폐기물명": waste_name, "성상": phase or "", "재질/특성": material or "",
    }
    rsp = client.responses.create(
        model="gpt-4o-mini",
        temperature=0.2,
        input=[{"role": "system", "content": sys}, {"role": "user", "content": str(user)}],
    )
...
if st.session_state.data_loaded:
    dfm, dfref = st.session_state.df_main, st.session_state.df_ref
    COL_WASTE, COL_METHOD = "폐기물 종류", "처리 방법"

    st.subheader("🔍 폐기물 정보 입력")
    c1, c2, c3 = st.columns([2, 1, 2])
    with c1: waste_name = st.text_input("폐기물명 *", placeholder="예: 폐유, 폐페인트")
        ...
        if not waste_name.strip():
            ...
            with st.spinner("🔍 검색중입니다..."):
                try:
                    ...
                    query_terms = normalize_query(waste_name, phase_input, material)
                    ...
                    if best_row is None:
                        ...
                        show_gate = contains_key(method_val, ALLOW_KEY) and not any(
                            contains_key(method_val, k) for k in EXCLUDE_KEYS
                        )

                        if show_gate:
                            # 2) 별표5 정확 일치: 사용자 입력어 우선 → 매칭 결과명 보조
                            ref = get_ref_exact(dfref, waste_name)
                            if not ref:
